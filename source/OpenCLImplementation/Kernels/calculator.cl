kernel void scalarMultHelp(global double* buffer, global double* buffer_small, __local double * target) {
...
kernel void scalarMultHelp2(global double* buffer, global double* buffer_small) {
	const size_t globalId = get_global_id(0);
...
kernel void ScalarMultiply( global double* A, global double* B, global double* buffer,
	...
	if (gid == 0) {
		...
		while (n >= 256) {
			...
			enqueue_kernel(get_default_queue(), CLK_ENQUEUE_FLAGS_NO_WAIT, ndr, 0, NULL, &marker_event, 
						   ^(local void* target){
						   		scalarMultHelp(buffer, buffer_small, (local double*)target);
						   	}, subthread_size*sizeof(double));
			n /= subthread_size;
			ndr = ndrange_1D(n, subthread_size);

			enqueue_kernel(get_default_queue(), CLK_ENQUEUE_FLAGS_NO_WAIT, 
						   ndr, 1, &marker_event, &marker_event2, 
						   ^{scalarMultHelp2(buffer, buffer_small); });

			enqueue_markel(get_default_queue(), 1, &marker_event2, NULL);
